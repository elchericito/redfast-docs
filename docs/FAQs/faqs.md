...
2. If all of the above is in place, check if the prompt is being delievered in the Ping call. You should see it under **Paths**:
   ...
   [block:image]{"images":[{"image":["https://files.readme.io/73b7658-image.png",null,""],"align":"center","border":true}]}[/block]
3. If the prompt is delivered but you still don't see it, check if there is an active experiment on the prompt with a control group and whether your user ID may be part of the control group. You can open the corresponding path and check the **action_group_name**. If it is set to "Control", this means that your user ID is held out of seeing the prompt by design.

   [block:image]{"images":[{"image":["https://files.readme.io/67a6aa2-image.png",null,""],"align":"center","border":true}]}[/block]
4. If the prompt is not delivered, check if there are any limits set on the prompt (i.e., impression limit/frequency cap/ delivery limit) or any user interaction settings (i.e., “Show prompt again after button 1 click or button 2 click” under the prompt editor) that may prevent you from seeing a prompt again if you've already interacted with it previously. 
   ...
   [block:image]{"images":[{"image":["https://files.readme.io/89184dd-image.png",null,""],"align":"center","border":true}]}[/block]
