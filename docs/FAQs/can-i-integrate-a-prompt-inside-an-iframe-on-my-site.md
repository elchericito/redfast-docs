...
Our goal is to open a popup to let the user know about a prompt once they have submitted their payment. We can do this by attaching a click handler to the Submit Payment button. We will replace ID_HERE below with the ID we collected in previous step. The code below will send a message to the parent container and Redfast will be listening for this specific event.
